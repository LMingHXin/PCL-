...
class cave(tk.Tk):
    ...
    def aboutit(self):#对应关于
        showinfo("关于", "版本:Dev-0.1.3-22-3-5\n最近更新:2022-3-5")
    ...
    def setup_UI(self):
        ...
        self.e1.grid(row = 1, column = 0, columnspan = 6, ipadx = 60)
        self.e2.grid(row = 3, column = 0, columnspan = 6, ipadx = 60)
        self.e3.grid(row = 5, column = 0, ipady = 100, columnspan = 6, ipadx = 60)
        tk.Label(self, text = 'QQ号').grid(row = 0, column = 0, columnspan = 6)
        tk.Label(self, text = '图片路径').grid(row = 2, column = 0, columnspan = 6)
        tk.Label(self, text = '回声洞留言内容').grid(row = 4, column = 0, columnspan = 6)
